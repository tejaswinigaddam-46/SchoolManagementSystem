                ...
                <div className="space-y-6">
                  ...
                  <div className="border-b pb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                     ...
                     <InputField label="Admission Date" name="admissionDate" type="date" required formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('admissionDate')} originalValue={originalStudentData?.admissionDate} showEditMode={showEditForm} />
                      ...
                      <InputField label="Registration No." name="registrationNumber" formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('registrationNumber')} originalValue={originalStudentData?.registrationNumber} showEditMode={showEditForm} />
                      <InputField label="Admission Type" name="admissionType" type="select" options={['New', 'Transfer', 'Re-admission']} formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('admissionType')} originalValue={originalStudentData?.admissionType} showEditMode={showEditForm} />
                      <InputField label="First Name" name="firstName" required formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('firstName')} originalValue={originalStudentData?.firstName} showEditMode={showEditForm} />
                      <InputField label="Middle Name" name="middleName" formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('middleName')} originalValue={originalStudentData?.middleName} showEditMode={showEditForm} />
                      <InputField label="Last Name" required name="lastName" formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('lastName')} originalValue={originalStudentData?.lastName} showEditMode={showEditForm} />
                      
                      <InputField label="Date of Birth" name="dateOfBirth" type="date" required formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('dateOfBirth')} originalValue={originalStudentData?.dateOfBirth} showEditMode={showEditForm} />
                      ...
                      <InputField label="Hostel Required" name="hostelRequired" type="select" options={['Yes', 'No']} formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('hostelRequired')} originalValue={originalStudentData?.hostelRequired} showEditMode={showEditForm} />
                    </div>
                  </div>
                  ...
                  <div className="border-b pb-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                      ...
                      <InputField label="Category" name="category" type="select" options={['General', 'OBC', 'SC', 'ST', 'EWS']} formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('category')} originalValue={originalStudentData?.category} showEditMode={showEditForm} />
                      
                      <InputField label="Blood Group" name="bloodGroup" type="select" options={['A+', 'A-', 'B+', 'B-', 'AB+', 'AB-', 'O+', 'O-']} formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('bloodGroup')} originalValue={originalStudentData?.bloodGroup} showEditMode={showEditForm} />
                      <InputField label="Height (cm)" name="height" type="number" formData={formData} handleInputChange={handleInputChange} isModified={modifiedFields.has('height')} originalValue={originalStudentData?.height} showEditMode={showEditForm} />
                          ...
                          <tbody className="bg-white divide-y divide-gray-200">
                            {formData.parents.map((parent, idx) => (
                              <tr key={idx} className="hover:bg-gray-50">
                                <td className="px-4 py-2 text-sm text-gray-900">{parent.firstName} {parent.lastName}</td>
                                <td className="px-4 py-2 text-sm text-gray-900">{parent.relation} {parent.isEmergency && <span className="ml-1 px-1.5 py-0.5 bg-red-100 text-red-700 text-[10px] font-bold rounded uppercase">Emergency</span>}</td>
                                <td className="px-4 py-2 text-sm text-gray-700">{parent.email}<br/><span className="text-xs text-gray-500">{parent.phone}</span></td>
                                <td className="px-4 py-2 text-right">
                                  <ActionButtonGroup className="justify-end">
                                    <EditButton onClick={() => handleEditParent(idx)} iconSize="h-3.5 w-3.5" className="p-1.5" />
                                    <DeleteButton onClick={() => handleRemoveParent(idx)} iconSize="h-3.5 w-3.5" className="p-1.5" requireConfirmation={false} />
                                  </ActionButtonGroup>
                                </td>
                              </tr>
                            ))}
                        ...
                        <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-200">
                          <div className="flex items-center">
                            <input 
                              ...
                              onChange={handleParentInputChange} 
                              className={`h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded ${modifiedParentFields.has('isEmergency') ? 'ring-2 ring-blue-200' : ''}`}
                            />
                            <label htmlFor="isEmergency" className={`ml-2 text-sm font-medium ${modifiedParentFields.has('isEmergency') ? 'text-blue-700' : 'text-gray-700'}`}>
              ...
              <div className="flex flex-wrap gap-2 w-full sm:w-auto">
                {selectedStudents.size > 0 && canExportStudents && (
                  <button
                    ...
                    className="flex-1 sm:flex-none bg-green-600 text-white px-3 py-2 rounded-lg hover:bg-green-700 transition-colors flex items-center justify-center gap-2 text-sm"
                  >
