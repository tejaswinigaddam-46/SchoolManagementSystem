...
const StudentBulkUpdate = ({ onUpdateSuccess, onCancel, campusId }) => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      const response = await studentService.bulkUpdate(formData);
      ...
      if (response.success === false && response.file) {
        ...
        // Fallback if backend returns summary JSON instead of file (should not happen based on service logic but good to have)
        toast.error(`Update completed with ${response.summary.failed} errors.`);
      } else {
        toast.success(response.message || 'Successfully updated students');
        setFile(null);
        // Reset file input
        const fileInput = document.getElementById('file-upload-update');
      ...
      toast.error(err.message || 'Failed to update students');
    } finally {
      setIsLoading(false);
    }
  };
